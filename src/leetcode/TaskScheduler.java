...
public class TaskScheduler {
    ...
    public static int leastInterval(char[] tasks, int n) {
        ...
        int max_key = 0;
        int max_value = 0;
        PriorityQueue<Pair<Character, Integer>> pq = new PriorityQueue<Pair<Character, Integer>>(new countComparator());
        ...
        for (Map.Entry<Character, Integer> entry : map.entrySet()) {
            ...
            pq.offer(new Pair<Character, Integer>(entry.getKey(), entry.getValue()));
        }
        pq.poll();
        
        if ((max_value + (max_value - 1) * n) >= total) {
            int[] count = new int[max_value];
            while (!pq.isEmpty()){
                Pair<Character, Integer> max = pq.poll();
                int temp = max.getValue();
                for (int i = 0; i < count.length; i++) {
                    if (count[i] + 1 <= n && temp != 0) {
                        count[i] += 1;
                        temp--;
                    }
                }                 
            ...
            if (count[count.length-1] == 0){
                return max_value + (max_value - 1) * n;
            } else {
                return max_value + (max_value - 1) * n + count[count.length-1];
            }
        } else {
            return total;
        }
...
class countComparator implements Comparator<Pair<Character,Integer>>{
	@Override
